...
const getDoctor = (req, res) => {
    ...
    .then((resp) => {
      res.send({
        type: "success",
        status_code: 200,
        data: { id:resp._id, firstName: resp.firstName, lastName: resp.lastName, email: resp.email, username: resp.username, specialization: resp.specialization, phoneNo: resp.phoneNo, address: resp.address }
      });
    })
    .catch((err) => {
...
const getSingleDoctor = async (req, res) => {
    ...
    res.send({ type: "error", message: "could not get single Doctor", error:err.message});
}
};
...
const DocRegister = async (req, res) => {
    try {
        ...
        if (find_doctor) {
            res.send({ type: "error", status_code: 401, message: "Doctor Already Exists" });
            return;
        }
        ...
        res.send({ type: "error", message: "could not create Doctor", error:err.message});
    }
};
...
const loginDoctor = async(req, res) => {
    try{
        ...
        if(doctor_email && doctor_password) {
            ...
            const find_password = await bcrypt.compare(doctor_password, find_doctor.password);
            if(!find_password){
    ...
    res.send({ type: "error", message: "Could not Login Doctor", error:err.message});
}
};  
...
const deleteDoctor = async (req, res) => {
    ...
    res.send({ type: "error", message: "Could not delete Doctor", error:err.message});
}
};
...
const updateDoctor = async (req, res) => {
    ...
    const updatedDoctor = await Doctor.findByIdAndUpdate(id, { name, email, phone, address, specialization }, { new: true });
    res.send({ type: "success", status_code: 200, data: updatedDoctor });
}


module.exports = {getDoctor, DocRegister, deleteDoctor, updateDoctor, loginDoctor, getSingleDoctor};
